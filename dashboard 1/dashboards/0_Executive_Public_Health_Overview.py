...
from src.styling import inject_css, page_header, kpi_card, section_title, PRIMARY, ACCENT, DANGER, SUCCESS, WARNING
...
with tab_summary:
    ...
    # KPI Row
    # ----------------------------------------------------------------------- #
    k1, k2, k3, k4, k5, k6, k7, k8 = st.columns(8, gap="medium")
    with k1:
        kpi_card("Population Under Surveillance", format_number(kpis["population_under_surveillance"]))
    with k2:
        kpi_card("Total Reported Cases", format_number(kpis["total_reported_cases"]))
    with k3:
        kpi_card("Active Cases", format_number(kpis["active_cases"]), color=WARNING)
    with k4:
        kpi_card("Recovered Cases", format_number(kpis["recovered_cases"]), color=SUCCESS)
    with k5:
        kpi_card("Deaths (Monthly)", format_number(kpis["deaths"]), color=DANGER)
    with k6:
        kpi_card("Case Fatality Rate", f'{kpis["case_fatality_rate"]}%', color=DANGER)
    with k7:
        kpi_card("Recovery Rate", f'{kpis["recovery_rate"]}%', color=SUCCESS)
    with k8:
        risk = kpis["public_health_risk_score"]
        risk_color = DANGER if risk >= 60 else WARNING if risk >= 35 else SUCCESS
        kpi_card("Public Health Risk Score", f"{risk}", color=risk_color)
...
with tab_surveillance:
    ...
    with sk1:
        kpi_card("Total Reported Cases", format_number(kpis["total_reported_cases"]))
    with sk2:
        kpi_card("Active Outbreaks Flagged", format_number(active_outbreaks), color=WARNING)
    with sk3:
        kpi_card("Outbreaks Controlled", format_number(controlled_outbreaks), color=SUCCESS)
    with sk4:
        kpi_card("Avg. Test Positivity Rate", f"{avg_positivity}%", color=DANGER)
    with sk5:
        kpi_card("Case Fatality Rate", f'{kpis["case_fatality_rate"]}%', color=DANGER)
    ...
    with sc1:
        ...
        default_diseases = (
            surv.groupby("disease_name")["total_reported_cases"].sum()
        ...
        if trend_s.empty:
            ...
            fig = px.line(trend_s, x="period", y="total_reported_cases", color="disease_name",
                          color_discrete_sequence=TREND_PALETTE, line_shape="spline")
            ...
            fig.update_layout(
                ...
                xaxis_title=None, yaxis_title="Total Reported Cases",
                xaxis=dict(showgrid=False), yaxis=dict(showgrid=True, gridcolor="#EEF2F5"),
                legend=dict(orientation="h", yanchor="bottom", y=1.02, xanchor="left", x=0),
            )
    ...
    with sc3:
        section_title("State × Disease Hotspot Matrix", "Total reported cases (darker = higher burden)")
        ...
        fig_heat = px.imshow(pivot, color_continuous_scale=NAVY_TEAL_SCALE, aspect="auto")
        fig_heat.update_layout(height=440, margin=dict(l=10, r=10, t=20, b=10))
